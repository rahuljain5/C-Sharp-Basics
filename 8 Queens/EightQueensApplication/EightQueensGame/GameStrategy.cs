	...
	class GameStrategy
	{
        ...
        private int getRow(int cellId)
		{
			/*
                Logic to be written ....
             */
			return (cellId - 1)/8;// Just for the heck of it
		}
		...
		public bool isValidPostion(int cellId)
		{
            ...
            for (int i = 0; i <= row; i++)
            {
                if (placedQueens[i, col] || (i <= col && placedQueens[row - i, col - i]) || (col + i < 8 && placedQueens[row - i, col + i]))
                {
                    isValid = false;
                }
            }
            placedQueens[row, col] = isValid;
            if (isValid) numQueens++;
            return isValid;
		}
